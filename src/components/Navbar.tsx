import { Menu, X, Globe, ShoppingCart, Eye, ChevronDown, Moon, Sun, Type, Search, User, Info, Phone, Package, ShieldCheck } from 'lucide-react';
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2 px-4">
                  <button 
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-colors ${isLargeText ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                  >
                ...
                {user && (
                   <div className="px-4 border-t border-gray-100 pt-3">
                     <button onClick={() => { setIsOpen(false); logout(); }} className="flex items-center gap-2 w-full px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest text-gray-400 hover:bg-gray-50 transition-colors">
                       <X className="w-3 h-3" /> {language === 'ru' ? 'Выйти' : 'Chiqish'}
                     </button>
                   </div>
                )}
              </div>
